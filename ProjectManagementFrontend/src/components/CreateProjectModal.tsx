  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        className={`theme-input w-full rounded-lg flex items-center gap-2.5 px-3 py-2.5 text-sm text-left transition-all focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/30 ${
          !selected ? 'theme-text-muted' : 'theme-text'
        }`}
      >
        {selected ? (
          <>
        ...
        <svg
          className={`h-3.5 w-3.5 shrink-0 theme-text-muted transition-transform duration-150 ${open ? 'rotate-180' : ''}`}
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                    value === u.id
                      ...
                      : 'theme-text hover:bg-violet-500/10 hover:text-violet-500'
                  }`}
                >
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold uppercase ${
                    value === u.id ? 'bg-white/20 text-white' : 'bg-violet-600/20 text-violet-500'
                  }`}>
...
export function CreateProjectModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (newTagDraft && newTagDraft.name.trim() && onCreateTag) {
        const tag = await onCreateTag(newTagDraft.name.trim(), newTagDraft.color)
        tagIds.push(tag.id)
      }
      await onSubmit({
        name: name.trim(),
  ...
  return (
    ...
    >
      ...
      >
        {/* Wrap everything in a form so footer button works */}
        <form onSubmit={handleSubmit} className="flex flex-col overflow-hidden h-full">
        {/* Header */}
          <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b theme-border shrink-0">
          <div>
            <h2 id="create-project-title" className="text-xl font-semibold theme-text">
              Create a Project
            </h2>
            <p className="mt-1 text-sm theme-text-secondary">
              Fill in the required details to set up your project.
            </p>
          </div>
          ...
          {/* Workspace */}
          <div>
            <label htmlFor="project-workspace" className="mb-1.5 block text-sm font-medium theme-text-secondary">
              Workspace
            </label>
            <select
          ...
          <div>
            ...
            <div className="flex gap-3">
              ...
              <input
                ...
                placeholder="e.g. SOC Setup, Website Vulnerability"
          ...
          {/* Status */}
          <div>
            <label htmlFor="project-status" className="mb-1.5 block text-sm font-medium theme-text-secondary">
          ...
          {/* Due Date */}
          <div>
            <label htmlFor="project-due-date" className="mb-1.5 block text-sm font-medium theme-text-secondary">
